...
{#if contacts.length === 0}
  <div class="flex flex-col py-12 text-gray-400">
...
{:else}
<ul class="divide-y divide-gray-100 rounded-lg shadow bg-white max-w-4xl">
  {#each contacts as contact}
    <li class="flex items-center gap-2 px-4 py-3 hover:bg-gray-50 transition group">
      <span class="flex-1 pl-2 pr-2 border-r border-gray-200">
        <div class="text-gray-500 flex justify-between"> 
          ...
          <div class="text-gray-500">{companies.find(c => c.id === contact.companyId)?.name}</div>
        </div>
      </span>
      <button class="btn btn-xs btn-secondary flex mr-4 cursor-pointer items-center gap-1 group/edit" title="Редактировать" on:click={() => onEdit(contact)}>
        Редактировать
      </button>
      <button class="btn btn-xs btn-error flex cursor-pointer items-center gap-1 group/delete" title="Удалить" on:click={() => handleDeleteClick(contact)}>
